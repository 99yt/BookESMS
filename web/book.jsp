...
<div class="clear"></div>
<!--主体main部分开始-->
<div id="bookContent" class="main page_width">
	<!--左边部分开始-->
	<div id="left">
    	<div id="book_sort">
            ...
            <div class="sort_list">
                <div class='item' v-for="category of bookCategories">
					<h3><b>&gt;</b><a :href="'book_list.jsp?parent_id='+category.category_id" class="dot"> {{category.name}}</a></h3>   </div>
        ...
        <div id="publishers">
            ...
            <ul>
            	<li v-for="publishing of Publishing" class='fore'><a href='#' target="_blank">{{publishing.name}}</a></li>
    ...
    <div id="right">
    	<div class="" id="report">
			...
			<div class="book_content">
				<ul><%--<c:forEach items="${bookNews}" var="newItem">
					<li>・<a  Target="_blank"  href="#">${newItem.title}</a></li>
				</c:forEach>--%>

					<li v-for="newItem of bookNews">・<a  Target="_blank"  href="#">{{newItem.title}}</a></li>
        ...
        <div id="week_sale">
            ...
            <div class="book_content">
                ...
                <ul class="rank">
					<li class='fore' v-for="active of Active">
                    	<span>{{active.id}}</span>
                        <div class='p-img'><a href='#' target='_blank'><img :src='active.src'/></a></div>
                        <div class='p-name'><a href='#'  target='_blank'>{{active.activeName}} </a></div>
                        <div class='pic_price'>定价：<del>￥{{active.price}}</del></div>
                        <div class='pic_price'>会员价：<strong>￥{{active.vipPrice}}</strong></div>
                    </li>
                    <%--<li>
                    	<span>2</span>
        ...
        <div id="new_sale">
            ...
            <div class="book_content">
                <ul class="rank">
					<li class='fore' v-for="a of hotActive">
                    	<span>{{a.id}}</span>
                        <div class='p-img'><a href='#' target='_blank'><img :src='a.src'/></a></div>
                        <div class='p-name'><a href='#'  target='_blank'>{{a.activeName}} </a></div>
                        <div class='pic_price'>定价：<del>￥{{a.price}}</del></div>
                        <div class='pic_price'>会员价：<strong>￥{{a.vipPrice}}</strong></div>
                    </li>
                   <%-- <li>
                    	<span>2</span>
                        <div class='p-img'><a href='#' target='_blank'><img src='img/book_52.jpg'/></a></div>
	...
	var vm=new Vue({
	 ...
	 $.ajax({
		 ...
		 success:function (result) {
				vm.$data.Publishing=result;
		 },
		 data:{"type":"queryPublishing"},
		 dataType:"json"
	 });
	 ...
	 $.ajax({
		 ...
		 type:"post",
		 success:function (result) {
				vm.$data.Active=result;
				vm.$data.hotActive=result;
		 },
		 data:{"type":"queryActive"},
		 dataType:"json"
	 });

	  alert("数据模型层，已经和视图进行数据同步");
	  $.ajax({
		  url:"book",
		  type:"post",
		  success:function (result) {
				alert(result)
			  	vm.$data.bookNews=result;
				console.log(result);
		  },
		  data:{"type":"queryBookNews"},
		  dataType:"json"
	  });
	//发送ajax请求，获取一级菜单信息
	$.ajax({
		...
		success:function(result){
			alert(result)
			console.log(result);
			vm.$data.bookCategories = result;

		},
		data:{"type":"queryBookCategory","parent_id":0},
		dataType:"json"
	});
